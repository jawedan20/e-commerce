...
const cartReducer = (state = initialState, action) => {
	...
	switch (action.type) {
		...
		case types.REMOVE_SELECT_CART:
			tempState = state.cartList.filter((item) => !py.includes(item.product.id));
			newcartState = CopyStateReplaceCartList(state, tempState);
			setCookie("cart", newcartState);
			return newcartState;
		...
		case types.FORM_CART:
			...
			newcartState = {
				cartList: [
					...state.cartList,
					{ product: { ...py.product }, quantity: py.quantity },
				],
			};
		...
		case types.UPDATE_CART:
			...
			const newProdCartStateUpdate = state.map((item) => {
				let itemFound = cartFormArr.find(
		...
		case types.DELETE_CART:
			const cartFormArrDel = Object.keys(action.payload).map(
				(key, index) => {
					return action.payload[key];
				}
			);
